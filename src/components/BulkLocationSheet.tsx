...
export default function BulkLocationSheet({ open, onClose }: BulkLocationSheetProps) {
  ...
  useEffect(() => {
    if (!open) {
      setStep('build'); setQuery(''); setResults([]); setQueue([])
      cascade.reset(); setAssignResults([]); setProgress({ done: 0, total: 0 })
      locCache.current.clear()
    }
  }, [open])
  ...
  // Pressing Enter treats input like a barcode scan
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl shadow-2xl max-h-[92vh] flex flex-col animate-slide-up">
        ...
        <div className="flex items-center justify-between px-4 pb-2 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-semibold text-gray-900">Bulk Location Assign</h2>
